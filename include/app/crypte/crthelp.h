...
namespace crt
{
    ...
    {
    public:
        static constexpr uint16_t crc16_be_table[256] = {
        ...
        static constexpr uint16_t def = 0xffff;

        static constexpr uint16_t crc16_byte(uint16_t crc, uint8_t it)
        {
            crc = crc16_be_table[(crc >> 8) ^ it] ^ (crc << 8);
            return crc;
        }

        static constexpr uint16_t crc16_arr(uint8_t *arr, size_t sz)
        {
            uint16_t crc = def;
            for (size_t i = 0; i < sz; i++)
            {
                crc = crc16_byte(crc, arr[i]);
            }
            return crc;
        }
    ...
    public:
        ...
        void crc(uint8_t item)
        {
            _crc = crc16_byte(_crc, item);
        }

        uint16_t crc_get() const { return _crc; }
        void crc_reset() { _crc = def; }
    };

    int32_t ftob(float f)
    {
        union ftob_t
        {
            float ff; // assumes sizeof(float) == sizeof(int)
            int ii;
        };
        ftob_t ftob = {};
        ftob.ff = f;
